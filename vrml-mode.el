;; $Id: vrml-mode.el,v 1.8 1997/12/19 13:01:48 parisjp Exp $
...
(defconst vrml-font-lock-keywords
  (list
   ...
   (cons (concat "\\(\\s-\\|^\\)\\("
                 ...
                 (mapconcat 'regexp-quote vrml-keyword-list "\\|")
                 "\\)\\(\\s-\\|$\\)")
         2)
   )
...
(if (fboundp 'font-lock-add-keywords)
    (font-lock-add-keywords 'vrml-mode vrml-font-lock-keywords))
...
Variables controlling indentation style:
  ...
  vrml-in-squarebrace-sexp-indent-level
    Indentation of VRML statements within square braces but not in
    'children' field.
  vrml-children-indent-level
    Indentation of VRML nodes within 'children' fields.
...
(defun vrml-brace-type (here &optional cont-sexp)
  (if (not cont-sexp)
      (save-excursion
        (let (parse-start state)
          (setq state (parse-partial-sexp (point-min) here 0)
                cont-sexp (car (cdr state)))
          (while (< (point) here)
            (setq parse-start (point)
                  state (parse-partial-sexp (point) here 0)
                  cont-sexp (car (cdr state)))))))

  (cond ((= (char-after cont-sexp) ?{)
         'curly)
        ((= (char-after cont-sexp) ?\[)
         (if (save-excursion
               (goto-char cont-sexp)
               (skip-chars-backward " \t\n")
               (forward-char -8)
             ...
             'children
           'square))
        (t (concat "vrml-brace-type: unrecognized char: "
                   (char-after cont-sexp))
           ())))
               ...
               (if (and (< (point) indent-point)
                   ...
                   (progn
                     ...
                     (while (and (< (point) indent-point)
                       ...
                       (forward-line))))
               (setq brace-type (vrml-brace-type indent-point containing-sexp))
               (cond
                ((eq 'square brace-type)
                ...
                ((eq 'children brace-type)
                 ;; we are in a children node. Use appropriate indent-level
                 (if (and (< (point) indent-point)
                          found-next-line)
                     (current-indentation)
                   (if t ; commands-p
                       (goto-char expr-start)
                     (goto-char containing-sexp))
                   (+ (current-indentation) vrml-children-indent-level)))
                (t
                 (error "calculate-vrml-indent: unknown brace type.")))))))))
        ...
        (if outer-loop-done
          ...
          (if (eolp)
            ...
            (cond ((not (numberp this-indent)))
                  ...
                  ((= (following-char) ?\])
                   (let ((type (vrml-brace-type (point))))
                   (setq this-indent
                         (- this-indent
                            (if (eq type 'square)
                                vrml-in-squarebrace-sexp-indent-level
